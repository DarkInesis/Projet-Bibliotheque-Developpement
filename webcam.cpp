...
webcam::webcam(class Window* window)
{
    ...
    if( !face_cascade.load( "../Projet-Bibliotheque-Developpement/resources/haarcascade_frontalface_alt2.xml" ) )
    {
...
Mat webcam::captureOrientation(){
    ...
    if (faces.size()==1)
    {
         ...
         if(abs(vect.x)<50 && abs(vect.y)<50){
             ...
             // Cas d'un mouvement léger
             Rect MatchedZone(p.x-templateWidth/2,p.y-templateHeight/2,templateWidth,templateHeight);
             if(vect.x>motionDetect_minLimit_horizontal){direction="droite";}
             else if(vect.x<-motionDetect_minLimit_horizontal){direction="gauche";}
             else if(vect.y>motionDetect_minLimit_vertical){direction="bas";}
             else if (vect.y<-motionDetect_minLimit_vertical){direction="haut";}
             else {
                 direction="null";
                 // Redefinition du model ( à revoir)
