...
Mat webcam::capture() {
      ...
      if (faces.size() == 1) {
            ...
            Point workingCenter(workingRect.x + workingRect.width / 2,
            ...
            if (abs(vect.x) < 30 && abs(vect.y) < 30) {
                  ...
                  if (vect.x > motionDetect_minLimit) {
                        direction = "droite";
                  } else if (vect.x < -motionDetect_minLimit) {
                        direction = "gauche";
                  } else if (vect.y > motionDetect_minLimit) {
                        direction = "bas";
                  } else if (vect.y < -motionDetect_minLimit) {
                        direction = "haut";
                  } else {
                  ...
                  Rect newModel(p.x - templateWidth / 2, p.y - templateHeight / 2, templateWidth,
                                templateHeight);
      ...
      while (waitKey(5) < 0) {
            ...
            if (faces.size() > 0) {
                  ...
                  for (int i = 0; i < (int)faces.size(); i++) {
                        ...
                        // Extract template image in oldFrame
                        // Do the Matching between the working rect in frame2 and the templateImage
                        // in frame1
                        matchTemplate(frame_gray, modelFace, resultImage, TM_CCORR_NORMED);
                        // Localize the best match with minMaxLoc
                        double minVal;
                        ...
                        if (abs(vect.x) < 30 && abs(vect.y) < 30) {
                              ...
                              std::cout << direction << endl;
                              // Redefinition du model ( à revoir)
                              Rect newModel(p.x - templateWidth / 2, p.y - templateHeight / 2,
                                            templateWidth, templateHeight);
